...
def keras_graphmodel():
    ...
    model = Model(inputs=seq_input,outputs = xs)
    return model
...
def train_and_test(X_train, y_train, X_test, y_test, n):
    ...
    hist = model.fit(X_train, y_train, validation_split=0.01, callbacks=[early_stopping], batch_size= 600, nb_epoch= 10)
...
model, probas  = train_and_test(X_train_rs, y_train, X_test_rs, y_test, n)
