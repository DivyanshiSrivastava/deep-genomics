...
def nn(xd,yd,zd,bd,fc_nodes,x,n,keep_prob):
    ...
    # final layer
    W_fc2, b_fc2 = variables([fc_nodes, 4], [4], 'W_fc2', 'b_fc2')
...
def run_conv(xd,yd,zd,bd,fc_nodes,dropout,n,x,y_, keep_prob, X_train, X_test, y_train, y_test):
    ...
    # balancing the unequal classes. 
    class_weight = tf.constant([[0.3,0.1,0.3,0.2]])
    ce = tf.nn.sigmoid_cross_entropy_with_logits(logits = y_conv, labels = y_)
    xent = tf.multiply(ce, class_weight, name="xent_raw") #shape [1, batch_size]
	...
	epochs = 0
	    ...
	    # print "Epoch %d, test accuracy %f" % (epochs, test_accuracy)
            ...
            with open(sys.argv[2], "a") as fp:
                read_multilabel.metrics(test_probas, y_test, 4, fp)
            epochs += 1
...
def main():
    ...
    X = read_multilabel.get_matrix(sys.argv[1] + ".shuf.seq")
    y = read_multilabel.get_labels(sys.argv[1] + ".shuf.conditions")
    ...
    y_ = tf.placeholder(tf.float32, shape = (None, 4))   # k = no_of_classes = 2
